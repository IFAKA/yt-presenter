...
(function() {
  ...
  function extractCaptionTracks() {
    try {
      ...
      console.warn('[YTPresenter] Failed to extract caption tracks:', e);
      return null;
    }
  }
  ...
  function extractChapters() {
    try {
      ...
      console.warn('[YTPresenter] Failed to extract chapters:', e);
      return null;
    }
  }
  ...
  function sendData() {
    ...
    console.log('[YTPresenter] Caption tracks:', tracks?.length || 0, '| Chapters:', chapters?.length || 0, '| Extended:', Object.keys(extended).length);

    window.postMessage({
      type: 'YTPRES_CAPTION_DATA',
      tracks: tracks,
      videoInfo: videoInfo,
      chapters: chapters,
      extended: extended,
  ...
  window.addEventListener('message', async (event) => {
    ...
    if (event.data?.type !== 'YTPRES_FETCH_TRANSCRIPT') return;

    const { videoId, languageCode, requestId } = event.data;
    try {
      const segments = await fetchTranscript(videoId, languageCode);
      window.postMessage({
        type: 'YTPRES_TRANSCRIPT_RESULT',
        requestId,
        success: true,
        segments,
      }, window.location.origin);
    } catch (err) {
      console.error('[YTPresenter] Transcript fetch failed:', err);
      window.postMessage({
        type: 'YTPRES_TRANSCRIPT_RESULT',
        requestId,
        success: false,
